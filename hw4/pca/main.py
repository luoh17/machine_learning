...
def main():
    ...
    d_demean = x_tr - mean
    val, vec = calculate_eigen(d_demean)
    val_sum = np.sum(val) 
    
    #a
    print(f"Lambda_i for i = {1, 2, 10, 30, 50} is :{[ val[i] for i in [0, 1, 9, 29, 49]]}")
    print(f"Sum of Lambda is {val_sum}.")
    ...
    for k in ks: 
        train_err.append(reconstruction_error(vec[:, :k],d_demean)) 
        test_err.append(reconstruction_error(vec[:, :k],x_test - mean)) 
    plt.figure()
    ...
    r = [] 
    for k in ks:
        r.append(1 - np.sum(val[:k]) / val_sum)
    plt.figure()
    plt.plot(r)
    plt.xlabel("K") 
    ...
    nr = 2
    nc = 5
    fig, axes = plt.subplots(nr, nc, figsize=(1.5* nc, 2 * nr))
    for i, ax in enumerate(axes.flatten()):
        ax.imshow(vec[:, i].reshape(28, 28), cmap='Blues') 
        ax.set_title(f'PCA Mode {i}')
    plt.tight_layout() 
    plt.show()

    # e 
    def reconstruct(digit_num):
        digit = x_tr[y_tr == digit_num][0] 

        k_s = [5, 15, 40, 100]
        names = ['Original'] + [f"k = {k}" for k in k_s] 
        r_d = [digit] + [reconstruct_demean(vec[:, :k],
        digit - mean) + mean for k in k_s]
        fig, axes = plt.subplots(1, 5, figsize=(1.5 * 5, 2 * 1))
        for i, ax in enumerate(axes.flatten()): 
            ax.imshow(r_d[i].reshape(28, 28), cmap='Blues') 
