...
class BinaryLogReg:
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        #raise NotImplementedError("Your Code Goes Here")
        exponential = np.exp(-y * (self.bias + X @ self.weight))
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        #raise NotImplementedError("Your Code Goes Here")
        log = np.mean(np.log(1+np.exp(-y*(self.bias + X @ self.weight))))
        reg = self._lambda * (self.weight.T @ self.weight)
        return  log + reg
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        #raise NotImplementedError("Your Code Goes Here")
        first  = np.mean((-y*(1-self.mu(X, y)))[:, None] * X, axis = 0)
        second = 2*self._lambda*self.weight
        return  first + second
    ...
    def misclassification_error(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        return 1-np.mean(y == y_prediction)
    ...
    def train(
        ...
        w_prev = self.weight + np.inf

        if m == "GD":
            while np.linalg.norm(self.weight - w_prev, np.inf) >= 1e-4 and count <= epochs:         
                count += 1
                w_prev = np.copy(self.weight)
            ...
            while np.linalg.norm(self.weight - w_prev, np.inf) >= 1e-4 and count <= epochs:         
                count += 1
                w_prev = np.copy(self.weight)
                batch_i = RNG.choice(n,batch_size)
                X_b = X_train[batch_i]
                y_b = y_train[batch_i]
                self.step(X_b, y_b, learning_rate)
                result["train_losses"].append(self.loss(X_train, y_train))
                result["train_errors"].append(self.misclassification_error(X_train, y_train))
                result["test_losses"].append(self.loss(X_test, y_test))
                result["test_errors"].append(self.misclassification_error(X_test, y_test))
...
if __name__ == "__main__":
    ...
    history = model.train(x_train, y_train, x_test, y_test, 0.01,  1000, 1, "GD")
    #history = model.train(x_train, y_train, x_test, y_test, 0.01,  500,1, "SGD" )
    #history = model.train(x_train, y_train, x_test, y_test, 0.01,  500, 100, "SGD")
    #print(history)
    ...
    plt.ylabel("Loss")
    plt.title('iterations vs losses')
    plt.legend()
    plt.show()
